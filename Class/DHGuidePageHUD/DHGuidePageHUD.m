...
- (instancetype)dh_initWithFrame:(CGRect)frame imageNameArray:(NSArray<NSString *> *)imageNameArray buttonIsHidden:(BOOL)isHidden {
    if ([super initWithFrame:frame]) {
        ...
        // 设置引导页上的跳过按钮
        UIButton *skipButton = [[UIButton alloc]initWithFrame:CGRectMake(DDScreenW-60, 20, 45, 26)];
        [skipButton setTitle:@"跳过" forState:UIControlStateNormal];
        skipButton.titleLabel.font = [UIFont systemFontOfSize:13];
        [skipButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
//        [skipButton setBackgroundColor:[UIColor darkGrayColor]];
...
//                UIImage *image = [UIImage imageNamed:@"guideImage_button_backgound.png"];
                
                UIButton *bgButton = [UIButton buttonWithType:UIButtonTypeSystem];
                bgButton.frame = CGRectMake(DDScreenW*0.3, DDScreenH*0.87, DDScreenW*0.4, DDScreenH*0.10);
//                [bgButton setImage:image forState:UIControlStateNormal];
//                bgButton.backgroundColor = [UIColor themeBlueColor];
                UIButton *enterButton = [UIButton buttonWithType:UIButtonTypeSystem];
                enterButton.frame = bgButton.frame;
...
//                [enterButton setTintColor:[UIColor whiteColor]];
        ...
        self.imagePageControl.pageIndicatorTintColor = [UIColor groupTableViewBackgroundColor];
        self.imagePageControl.currentPageIndicatorTintColor = [UIColor themeBlueColor];
...
- (void)buttonClick:(UIButton *)button {
    ...
    //替换版本号
    NSString *currentBuild = [MRAPPInfo build];
    [[NSUserDefaults standardUserDefaults] setValue:currentBuild forKey:@"CFBundleVersion"];
